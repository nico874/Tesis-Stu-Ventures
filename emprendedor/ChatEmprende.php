...
$sqlMensajesTotal = "SELECT COUNT(*) idCorres from chats_correspon Where De = '$_SESSION[idUsuarioEm]' ";
...
$sqlUpdateChat = "UPDATE chat set Estado = '0' WHERE Recibe_Usuarioid= '$_SESSION[idUsuarioEm]' AND idCorres = '$idCorres'";
...
<body>
    ...
    <section class="contenedorChat"><br>
        <div class="container">
            <div class="person2">
                <a href="Mensajes.php"> <i class="bi bi-arrow-left-circle"></i> </a>
            </div>
            ...
            <div class="contenedorleft">
                ...
                ?>
                    <div class="bg-light p-4 d-flex flex-column align-items-center  p-3 py-4">
                        ...
                        <span class="text-black-50 ">Precio:<?php echo  $columnaPubliC['Precio'] ?> </span>
                    </div>
                <?php } ?>
            </div>
            <div class="user_header">
                ...
                if (isset($_GET['idCorres'])) {
                    ...
                    $sqlPubliCorreDe = "SELECT * from chats_correspon WHERE idCorres ='$_GET[idCorres]'";
                    $resukDe = mysqli_query($conexion, $sqlPubliCorreDe);
                    $filaPubliCorreDe = mysqli_fetch_array($resukDe);

                    $sqlPubliCaUser = "SELECT * from usuario Where idUsuario = '$filaPubliCorreDe[De]'";
                    $resultaPubliCaUser = mysqli_query($conexion, $sqlPubliCaUser);
                    $columnaPubliCaUser = mysqli_fetch_array($resultaPubliCaUser);
                ...
                ?>
                    ...
                    <div class="chat-about">
                        ...
                        <?php if ($columnaPubliCaUser['Tipo_usuario'] == 3) { ?>
                            <h6>
                                Usuario Consultante: <b class="text-danger">ADMINISTRADOR</b> <?php echo $columnaPubliCaUser['Nombre'] ?> <?php echo $columnaPubliCaUser['Apellido_Paterno'] ?>
                            </h6>
                        <?php } else { ?>
                            <h6>Usuario Consultador: <?php echo $columnaPubliCaUser['Nombre'] ?> <?php echo $columnaPubliCaUser['Apellido_Paterno'] ?> </h6>
                        <?php   } ?>
                    </div>
                <?php } ?>
            </div>
            <div class="historiaChat" id="historiaChat">
                ...
                if (isset($_GET['idCorres'])) {
                    ...
                    if (mysqli_num_rows($resultChat) == 0) {
                        ...
                        $NoHay = "";
                        $idEm =  $filaPubliCorreDe['De'];
                        while ($filasChat = mysqli_fetch_array($resultChat)) { ?>

                            <?php if ($filasChat['Envia_Usuarioid'] == $_SESSION['idUsuarioEm']) { ?>
                            ...
                            <?php } elseif ($filasChat['Envia_Usuarioid'] == $idEm) { ?>
                                <div class="message-feed media" id="mensa2">
                                    <div class="message-container2">
                                        <div class="media-body">
                                            <img src="../img/usuario1.png" alt="avatar" class="img-avatar">
            ...
            <div class="msb-reply">
                <form method="GET" class="needs-validation" id="FrmAJAX" novalidate>
                    ...
                    <input type="hidden" name="idUsuario" value="<?php echo  $filaPubliCorreDe['De'] ?>">
                    <?php if ($acciDesabi == "") { ?>
                        <div class="input-group-prepend">
                            <div class="row" id="MiInput">
                                <textarea name="mensaje" class="form-control" id="exampleFormControlTextarea1" placeholder="Escribe tú mensaje aqui..." required style="max-height: 90px;"></textarea>
